                ...
                if(m) {
                    ...
                    switch(pseudo) {
                        ...
                        case "visible": nodes = filter(nodes, function() {
                                ...
                                // This doesn't work:
                                // "The attribute visibility is not currently supported in FBJS!"
...
FBjqRY.fn = FBjqRY.prototype = {
    ...
    action: function(value) {
        if(typeof value != 'undefined') {
            each(this.nodes, function() { this.setAction(value); });
            return this;
        }
        return this.nodes[0].getAction();
    },

    href: function(value) {
        if(typeof value != 'undefined') {
            each(this.nodes, function() { this.setHref(value); });
            return this;
        }
        return this.nodes[0].getHref();
    },
    ...
    filter: function(fn) {
        var remainingNodes = [];
        if(typeof(fn) == "string") {
            // this was blowing up, bad closure? had to use a different name to work
            // fn = function(node) { return node.is(fn); };
            var func = function(node) { return node.is(fn); };
        } //else it should already be a function
        for(var i = 0, length = this.nodes.length; i < length; i++) {
        	  var tmp = FBjqRY(this.nodes[i]);
            if(func(FBjqRY(this.nodes[i]))) { remainingNodes.push(this.nodes[i]); }
        }
    ...
    // TODO:: add jquery boolean param for initial hidden value
    toggle: function() {
        ...
        each(this.nodes, function() {
            if (length > 0) {
              ...
              FBjqRY(this).click(function() {
            	...
            	var state = FBjqRY(this).is(":hidden");
...
FBjqRY.extend({
    ...
    //load: function(url, data, callbacks) {},  //-- defined in EVENTS
    //get:  function(url, data, callbacks) {},  //-- defined in CORE
    
    genericAJAX: function(url, data, callbacks, response_type, retryCount) {
        retryCount = retryCount || 3;
        if(!callbacks && FBjqRY.isFunction(data)) {
            callbacks = {
            	success: data
            };
            data = null;
        ...
        callbacks = this.checkCallbacks(callbacks)
        ...
        switch(response_type) {
          ...
          case 'JSON':
            ...
            break;
          default:
            notSupported("Unsupported response type: " + response_type);
            break;
        }
        ...
        /*
        // removed ajax retries for now
        ajax.onerror = function() {
            console.error("AJAX error occurred! Retrying...");
            if(retryCount > 0) { FBjqRY.genericAJAX(url, data, callbacks, type, retryCount-1); }
        };
        */
        // _token and _hostname are rails specific variables for facebooker that should be declared above this file's inclusion
        data = data || {};
        if (typeof _token != 'undefined') {
        	data['authenticity_token'] = _token;
        }
        if (url.indexOf('http') == -1 && typeof(_hostname) != 'undefined') {
        	url = _hostname + url;
        }
    ...
    getJSON: function(url, data, callbacks) {
        this.genericAJAX(url, data, callbacks, 'JSON');
    },

    getScript: function(url, callbacks) { notSupported("We can't pull in scripts dynamically using FBJS"); },

    post: function(url, data, callbacks, response_type) {
    	  response_type = response_type || 'FBML'
        this.genericAJAX(url, data, callbacks, response_type);
    },

    update: function(target, url, data, callbacks, response_type) {
    	response_type = response_type || 'FBML'
      if(!callbacks && FBjqRY.isFunction(data)) {
          callbacks = {
            success: data
      ...
      callbacks.success = (callbacks.success == null) ?
        update_element :
        this.chainMethods([update_element, callbacks.success]);
    ...
    chainMethods: function(callbacks) {
